...
ansible all -i localhost, -c local -m template -a "src=templates/galaxy/config/job_conf.xml.j2 dest=job_conf.xml" --extra-vars=@host_vars/pawsey_job_conf.yml --extra-vars=@.ci/test_vars/job_conf_test_vars.yml

echo -e "\nRunning job destination check\n"
SUCCESS_RESPONSE="Configuration is valid!"
VALIDATION_OUTPUT="$(python dynamic_tool_destination.py -c files/galaxy/dynamic_job_rules/pawsey/dynamic_rules/tool_destinations.yml -j job_conf.xml)"
